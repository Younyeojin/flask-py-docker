...
class Population(object):
    ...
    def find_similer_area(self, name: str):
        mn = 1
        result = 0
        home = []
        for i in self.data:
    ...
    def show_plot_similar_cities(self, name:str, home: [], away:[]):
        plt.style.use('ggplot')
...
if __name__ == '__main__':
    pop = Population()
    pop.read_data()
    # name = input('인구구조가 알고 싶은 지역의 이름(읍면동 단위)을 입력해 주세요')
    pop.find_home('필동')
    pop.home = pop.list_to_array(pop.home)
    pop.find_similer_area('필동')
    pop.show_plot_similar_cities('필동', pop.home, pop.away)
